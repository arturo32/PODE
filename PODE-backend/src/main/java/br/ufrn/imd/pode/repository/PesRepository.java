...
public interface PesRepository extends GenericRepository<Pes, Long> {
	...
	@Query(value = "SELECT\n" + 
			...
			"GROUP BY pes.id\n" +
			"ORDER BY pes.id ASC;\n", nativeQuery = true)
	public List<PesChobCumpridaResult> findPesComChobCumpridaByVinculo(@NotNull long vinculoId);
	...
	@Query(value = "SELECT\n" + 
			...
			"		INNER JOIN disciplina AS disciplina_secundario\n" + 
			"			ON disciplina_periodo_secundario.disciplina_id = disciplina_secundario.id\n" + 
			"		WHERE vinculo_secundario.id = 0\n" + 
			"	)\n" + 
			"GROUP BY pes.id\n" +
			"ORDER BY pes.id ASC;\n", nativeQuery = true)
	public List<PesChopCumpridaResult> findPesComChopCumpridaByVinculo(@NotNull long vinculoId);
